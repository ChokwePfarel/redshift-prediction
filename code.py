...
try:
    ...
    # Extract redshift (redshift is the 1st column, index 0)
    try:
        if M.ndim > 1:
...
# Ensuring that F and redshift match size after reduction
...
# Calculating the Pearson correlation coefficient (r) on the test set
r, p_value = pearsonr(y_test, y_pred_test)
print(f"Correlation Coefficient (r) on Test Set: {r:.4f}")
print("--------------------------------------------------")

# --- Visualization: Spectra Colored by Predicted Redshift ------------------
...
# --- Visualization: PCA Scatter (Predicted Redshift) ------------------
...
# --- Visualization: Predicted vs Actual Redshift (Random Forest) ------------------
# Use the test set actual and predicted values from the Random Forest model
...
plt.title('Predicted vs Actual Redshift (Random Forest)')
plt.legend()
plt.grid(True)
plt.show()
